...
private:
	...
	{
		// DbgPrint("[.] INFO: ContiguousMemory() called.\n");
	}
...
public:
	...
	{
		// DbgPrint("[.] INFO: ~ContiguousMemory(%d) called.\n", m_virtualAddress != nullptr);
	...
	static Optional<ContiguousMemory> allocate(const size_t size)
	{
		...
		return Optional<ContiguousMemory>(static_cast<ContiguousMemory&&>
			(ContiguousMemory(virtualAddress, physicalAddress)));
	}
};
